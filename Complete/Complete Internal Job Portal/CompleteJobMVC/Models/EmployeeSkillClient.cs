...
namespace EmployeeSkillMVC.Models
{
    public class EmployeeSkillClient
    {
        ...
        public async Task GetToken(string userName, string roleName)
        {
            token = await webApi.GetStringAsync("http://localhost:49898/AuthSvc/?userName=" + userName + "&role=" + roleName + "&key=One piece is real");
            webApi.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
        }
        public async Task DeleteEmployeeSkillAsync(string SkillId,string EmpId)
        {
            await webApi.DeleteAsync("Delete/" + EmpId+ SkillId);
        }
